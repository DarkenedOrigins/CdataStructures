...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if( ptr->cutline == UNDEFINED_CUTLINE ) return 1;
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(a->left != NULL){
		if (is_in_subtree(a->left, b) == 1)return 1;
	}
	if(a->right != NULL){
		if (is_in_subtree(a->right, b) == 1)return 1;
	}
	return 0;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // Internal node
  // Assume its a vertical cutline for now
  // assign the data to the module pointed by ptr
  ptr->module = NULL;
  ptr->cutline = V;
  ptr->parent = par;
