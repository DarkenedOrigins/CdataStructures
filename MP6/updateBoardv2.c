int out_of_bounds(int index, int boardRowSize, int boardColSize);
/*
 * countLiveNeighbor
 * Inputs:
...
int countLiveNeighbor(int* board, int boardRowSize, int boardColSize, int row, int col){
	// Check boundary conditions as we go!
  	int neighbors=0;
	int index;
	index = row*boardColSize+col-1;
	if (!out_of_bounds(index, boardRowSize, boardColSize)){
	...
	index = (row-1)*boardColSize+col;
	if (!out_of_bounds(index, boardRowSize, boardColSize)){
		neighbors += board[index];
	}
	index = (row-1)*boardColSize+col-1;
	if (!out_of_bounds(index, boardRowSize, boardColSize)){
		neighbors += board[index];
	}
	index = (row-1)*boardColSize+col+1;
...
int out_of_bounds(int index, int boardRowSize, int boardColSize){
	...
	// returns 1 if it is out of bounds, 0 otherwise
	int max = boardRowSize*boardColSize - 1;
...
int V2 (int row, int col, int boardRowSize, int boardColSize){
	if (row < 0 || row > boardRowSize-1) return 0;
	if (col < 0 || col > boardColSize-1) return 0;
	return 1;
}
